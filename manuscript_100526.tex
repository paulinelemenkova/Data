...
\Title{Artificial Intelligence and Geospatial Analytics for Seismic Hazard Assessment: A Review of Advancements in the Eastern Marmara Region, Türkiye}

% MDPI internal command: Title for citation in the left column
\TitleCitation{Artificial Intelligence and Big Data Analytics for Seismic Hazard Assessment: A Review of Advancements in the Eastern Marmara Region, Türkiye}
...
Systems of earthquake hazard monitoring aim at predicting earthquakes before they appear. Geophysical centres of data analysis detect signals in seconds across thousands of daily records from seismic sensors worldwide. Creating systems of seismic hazard monitoring that process millions of data points daily enables geophysical companies to predict earthquake events. However, processing big data in seismology becomes a challenge and requires special tools of their handling. This becomes possible through techniques of Artificial Intelligence (AI) and cloud computing. While traditional methods of data analysis in geophysics are not efficient to rapid real-time seismic data processing, the breakthrough comes when millions of earthquake signals recorded worldwide are treated as a big data massifs.

The fundamental concepts and characteristics of big data systems in seismology include datasets collected from geophysical sensors worldwide for earthquake prognosis and analytics. The difference between traditional geophysics and current challenges of big data relies on understanding that big data is not only having larger volume of signal records and obtaining more information from them. It is rather recognising different approaches to data challenges in geophysics which now require principally novel methods of their storage, processing and analysis. 

The five Vs of big data in seismology —volume, velocity, variety, veracity and value—gain advantages over traditional local datasets, typical for regional geophysical data centres. While traditional seismic databases keep gigabytes of information, big data systems regularly process petabytes of recorded data from sensors worldwide. Another issue is related to the speed required for processing and analysis of big data in seismology: modern systems of earthquake predictions handle real-time data streams while providing immediate insights for decision-making and evaluating seismic risks. 
...
The variety in big data in geophysics is characterised by a spectrum of the heterogeneous data structures. The complexity of formats arise due to the diverse data structures and sources of data that should be integrated and analysed together. Even if the data is available as a CSV in clear rows and columns, that file is often presents a wrapper for a mix of information pulled from across geophysical sensors. Finally, the veracity of large volumes of data is related to data quality and integrity. When dealing with web-sourced datasets on seismic records, these might include incomplete signals, duplicate entries of events, inconsistent formatting of diverse data, occasionally broken sensors, or outdated information.

The five Vs create interconnected challenges in modern seismology that must be addressed. Conventional methods of seismic data analysis mostly rely on manual examination of datasets, ignoring massive volumes of historical seismic data available at archives and new records flowing through sensors and stored in online geophysical repositories. The analysis of such large massifs of data helps discovering anomalies in mantle processes with better precision and speed. Such insight can be derived from analysing real-time data characterising location and magnitude of seismic events, unstructured cartographic data, and all relevant geological characteristics that define the unique value of regional data. As a result, big data analytics in seismology prevent failures in prognosis through predictive analytics that processes sensor data from thousands of machines simultaneously. 
...
The country is located on the active tectonic faults, which contribute to the initiation of multiple earthquakes \cite{Burton1984,Kandregula2024,Yao2024}. Given this context, big data monitoring is essential for seismic risk assessment of the country, underscoring the need for ongoing research and the development of methodologies that can enhance societal safety.
...
Today, geophysics operates at a complex intersection where the volume of data is increasing, but the physics-based models used to interpret that data are hitting computational and theoretical ceilings. As industry focuses shift toward high-density data acquisitions for seismic signals or geothermal exploration, the scale of the data has made traditional manual workflows impossible. Hence, modern geophysics faces a data deluge from high-density nodal sensors that overwhelms traditional Input-Output (I/O) bandwidth and storage, making real-time "edge" processing essential but difficult to implement. 
...
The primary technical hurdle lies in integrating physics-based wave equations with AI. While deep learning (DL) offers speed, it often lacks the geological rigour of traditional inversion, which remains plagued by the "non-uniqueness" problem where multiple models fit the same data. Furthermore, legacy approaches struggle with the physical trade-off between depth and resolution—especially in complex sub-salt or sub-basalt environments—and the reliance on manual quality control has become a significant bottleneck that cannot scale with modern, massive datasets.

%----------- subsection ----------->
\subsection{Hybrid computing: HPC and Cloud mosaic}
One of the most demanding data processing tasks in geophysics includes those involving the massive datasets from nodal sensors and the complex inversions, such as Full Waveform Inversion (FWI). Since nodal datasets can reach petabyte scales, geophysicists no longer move all data to a single location. Instead, they use a hybrid mosaic approach. The principle of hybrid approach consists in taking the advantages of both HPC and cloud computing.

Handling the big data in geophysics requires a shift from linear, sequential processing to a hybrid architecture that blends high-performance computing (HPC) with cloud-native intelligence. To process massive nodal datasets and complex inversions, geophysicists utilise a hybrid architecture that combines the raw power of GPU-accelerated high-performance computing (HPC) clusters with the elastic scalability of the cloud, effectively moving the processing code to the data to avoid massive transfer bottlenecks \cite{}. 

For example, FWI is a compute-intensive task which simulates seismic waves through every cubic meter of the earth and requires heavy lifting on HPC. The hybrid infrastructure accelerates high-resolution FWI by embedding physical laws directly into AI algorithms, since it is driven by Physics-Informed Neural Networks (PINNs) \cite{}. The FWI remains on GPU-accelerated HPC clusters because of the tight coupling required between processors. In turn, bursty tasks, such as initial seismic signal denoising and geometry correction of the dataset are offloaded to the cloud which profits from its elasticity. This allows teams to spin up 10,000 nodes for a single weekend of processing without owning the hardware. For remote nodal arrays, smart nodes now perform initial data compression and basic quality checks at the source, transmitting only the most relevant seismic signal back to headquarters, Table \ref{tab02}. 

\begin{table}[H] 
\caption{Summary of modern big data processing in geophysics.\label{tab02}}
%\newcolumntype{C}{>{\centering\arraybackslash}X}
\begin{tabularx}{\textwidth}{CCC}
%\toprule
\textbf{Task} & \textbf{2026 Strategy}	& \textbf{Technology Used}\\
%\midrule
Nodal Data Ingest & Decoupled Streaming & Cloud Object Storage (S3/Azure Blob)\\
Velocity Modeling & PINN-based Inversion & NVIDIA H100/H200 Clusters\\
Noise Removal	 & Deep Learning Autoencoders & CNN-based Denoising\\
Reservoir Monitoring	 & Digital Twins	& Real-time Edge Computing
%\bottomrule
\end{tabularx}
\end{table}
\unskip
...
The most demanding inversions are now handled by PINNs. Unlike traditional AI, which might suggest geologically impossible structures, PINNs embed the actual wave equations (the physics) into the neural network's loss function. The Benefit of using PINNS is that they can "fill in the gaps" of sparse nodal data by ensuring every predicted pixel adheres to the laws of thermodynamics and fluid dynamics \cite{}. Moreover, AI models like PhaseNet (e.g., see \cite{,,}) are used to automatically "pick" millions of seismic arrivals from nodal data—a task that would take human teams years to complete manually.
...
\begin{itemize}
	\item Compressed sensing. This method allows to reconstruct a high-resolution image from fewer sensors by exploiting the inherent "sparsity" of geological structures \cite{,,}.
	\item Asynchronous inversion. Instead of waiting for a total survey to be completed, large datasets is processed in "rolling windows", allowing for near-real-time updates of the subsurface geological model during an active survey \cite{,,}.
\end{itemize}
...
Distributed computing frameworks are designed to process massive datasets such as big data in high speed. One example of such toolkits in open-source used in geophysics is PySpark -- the Python API for Apache Spark. In the context of earthquake prognosis and seismic signal processing, employing such tools as PySpark enables the transition from analyzing isolated events to processing continuous waveform data across global sensor networks. For instance, by leveraging DataFrame transformations to window high-frequency sampling data and running analysis queries on historical catalogs, geophysicists can identify subtle precursory patterns—such as tectonic tremor or seismic swarming—that would overwhelm traditional single-node systems. 

This distributed approach is vital for collaborative seismic monitoring, allowing teams to synchronise real-time data streams with complex signal-to-noise ratio (SNR) enhancement algorithms. For instance, employing PySparrk to large dataset processing in geophysics supports the deployment of recursive neural networks (RNNs) for pick-time estimation \cite{}, and the execution of massive cross-correlation operations required for ambient noise interferometry \cite{}, all within a scalable cloud architecture.

The primary collaboration challenges stem from the "Domain vs. Code" gap, where specialized physical algorithms must be translated into scalable code, and the technical friction of handling "heavy" legacy data formats like SEG-Y (Society of Exploration Geophysicists - Y format alongside modern cloud-native formats. These teams often face "Black Box" skepticism, where scientific rigor clashes with opaque machine learning results, and resource contention over the massive computational power required for tasks like 3D seismic inversion. Using a unified platform like Azure Databricks helps mitigate these issues by providing a shared workspace that harmonizes domain expertise with distributed computing.

\subsection{Geophysical data hubs in Turkey: Distributed architecture}
The distributed approach to data processing in earthquake monitoring centres corresponds to its current organisational structure of geophysical data centres. Since Turkey is home to some of the most advanced geophysical data centres in the Mediterranean and Middle East, its primary focus is on seismic monitoring due to the country’s unique position on major fault lines. Data centres include a network of centres processing and analysing seismic and earthquake data. The following institutions as the primary hubs for geophysical data collection, processing, and distribution:

\begin{itemize}
  \item KOERI (Kandilli Observatory and Earthquake Research Institute): Based at Boğaziçi University, this is arguably the most famous geophysical data center in Turkey. It operates the National Earthquake Monitoring Center (NEMC) and is the primary source for real-time seismic and tsunami data in the Marmara region.  

    \item AFAD (Disaster and Emergency Management Presidency): AFAD operates the Earthquake Data Center System of Turkey (TDVMS). It manages a massive network of over 1,100 stations and serves as the official government authority for disaster response data.  

   \item MTA (General Directorate of Mineral Research and Exploration): Located in Ankara, MTA maintains the most comprehensive databases for geological and geophysical surveys related to mineral deposits, geothermal energy, and active fault mapping across Turkey.  

    \item TPAO (Turkish Petroleum Corporation): TPAO holds the country’s largest private/government-held collection of 3D and 4D seismic reflection data (often in SEG-Y format) used for hydrocarbon exploration in the Black Sea and Mediterranean.

    \item TÜBİTAK MAM (Marmara Research Center): Its Earth and Marine Sciences Institute focuses on R\&D data, including high-resolution soil analysis, liquefaction potential, and marine geophysics using specialized research vessels.  

    \item Belbaşı Nuclear Tests Monitoring Center: Run by KOERI in Ankara, this is a specialized facility that collects geophysical data to detect underground nuclear explosions globally, contributing to the Comprehensive Nuclear-Test-Ban Treaty (CTBT).  

    \item ITU (Istanbul Technical University) - Geophysics Dept: ITU houses several high-level laboratories, including the Prof. Dr. N. Canıtez Data Processing Lab, which handles advanced academic seismic imaging and electromagnetic data.  

    \item METU (Middle East Technical University) - Earth System Science: METU focuses on multi-disciplinary data, integrating geodetic data (GPS/GNSS) with seismic records to study crustal deformation and plate tectonics.

    \item Dokuz Eylül University - Earthquake Research and Implementation Center (DAUM): Based in İzmir, this center is the primary data hub for seismic activity and fault analysis in the Aegean region.

    \item Kocaeli University - Department of Geophysics: Following the 1999 earthquake, this department became a major center for urban geophysics data, focusing on site-response analysis and seismic microzonation for the industrial heartland of Turkey
\end{itemize}

\subsection{Geophysical data centres: Collaboration challenges and technical silos}
...
\subsection{Scaling seismic resilience: insights in Turkey}
...
\begin{figure}[H]
    \begin{center}
	\hspace{-35pt}\includegraphics[width=13.0cm]{Fig_05.png}
    \end{center}
