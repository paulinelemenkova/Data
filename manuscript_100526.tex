...
\begin{table}[H]
\caption{State-of-the-Art Big Data Processing in Geophysics (2026 Framework)}
\label{tab:geophysics_2026}
\begin{tabularx}{\textwidth}{@{} >{\hsize=0.8\hsize}C >{\hsize=1.2\hsize}C >{\hsize=1.0\hsize}C @{}}
\toprule
\textbf{Geophysical Task} & \textbf{2026 Advanced Strategy} & \textbf{Technology Stack} \\ \midrule
...
\textbf{Seismic Signal Enhancement} & Self-supervised Deep Learning Autoencoders for 5D interpolation and ghost reflection removal. & Transfomer-based Denoising, GANs \\ \addlinespace
...
\textbf{Multi-Physics Integration} & Joint inversion of gravity, magnetic, and seismic data via Large Language Models (LLMs) for interpretation. & Foundation Models, Graph Neural Networks (GNNs) \\ \bottomrule
\end{tabularx}
\end{table}
...
\subsection{Geophysical data hubs in Turkey: Distributed architecture, collaboration and technical silo}
...
\begin{table}[H]
\caption{Principal Organizations and Big Data Activities in Marmara Region Seismology and EEW}
\label{tab03}
\begin{tabularx}{\textwidth}{@{} X >{\raggedright\arraybackslash}p{3.5cm} >{\raggedright\arraybackslash}p{6.5cm} @{}}
\toprule
...
\textbf{MTA} \newline \textit{Est. 1935 (Ankara)} & Curates the Active Fault Map of Turkey. & Big data activities focus on high-resolution GIS integration of paleoseismological data and surface rupture mapping to inform long-term probabilistic hazard models. \\ \addlinespace
...
\textbf{Kocaeli University} \newline \textit{Post-1999 expansion} & Center for urban geophysics. & Processes massive site-response and microzonation datasets for the Marmara industrial corridor, focusing on soil-structure interaction during high-amplitude shaking. \\ \bottomrule
\end{tabularx}
\end{table}
