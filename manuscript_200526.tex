...
\abstract{The Marmara region of Türkiye, situated along the complex North Anatolian Fault Zone (NAFZ), represents one of the most seismically active and monitored zones globally. Given the region's high vulnerability and the catastrophic impacts of historical events, such as the 1999 İzmit and 2023 Kahramanmaraş sequences, there is a critical need for advanced seismic hazard risk assessment (SHRA). This research explores the paradigm shift from traditional geophysics to big data seismology, characterized by the "Five Vs": volume, velocity, variety, veracity, and value. The study highlights how the integration of Artificial Intelligence (AI), cloud computing, and high-performance computing (HPC) enables the real-time processing of petabyte-scale sensor streams of seismic signals for geophysical modelling. We discuss the implementation of as architecture of cloud-native platform of Azure Databricks, advantages of data pipelines using Apache Kafka and Apache Spark for big data processing on cloud platforms to transition raw seismic waveforms into actionable insights through structured data tiers. Key technological advancements, including Physics-Informed Neural Networks (PINNs) and Deep Learning models like PhaseNet, are analyzed for their ability to enhance Earthquake Early Warning (EEW) systems by providing sub-second phase picking and automated event detection.  By addressing computational bottlenecks through hybrid mosaic architectures and edge computing, this framework aims to maximize the efficiency of warning for Istanbul’s critical infrastructure. Ultimately, this paper provides a roadmap for bridging the gap between traditional data analysis and predictive analytics to enhance societal safety in the Marmara region. This contribution underscores the vital role of interdisciplinary data management and high-performance computing in maximizing the "Golden Seconds" of warning, ultimately improving the long-term resilience of seismically active urban zones of Istanbul, Türkiye.}
...
During the 19th century, the region faced intensified activity. The 1881 Chios-\c{C}e\c{s}me earthquake stands out as a catastrophic event that caused thousands of fatalities and significant coastal changes \cite{ambraseys2000}. Moving into the 20th century, the North Anatolian Fault Zone began a well-documented westward-migrating sequence of large ruptures. A significant event in this timeline was the Amorgos earthquake in 1956, which struck the Southern Aegean \cite{okals2009}. This earthquake generated a tsunami with a wave height of 2.5 m, leading to severe flooding over an area with a depth of 1.5 km on the island of Kalimnos. Additionally, this seismic activity caused substantial material damage to several nearby islands, including Ios, Nisyros, Patnos, Tilos, and Lipsos \cite{papadopoulos2007}.

The tsunami wave reached 1m in Fethiye, and the flood depth was observed to be 250m. On 17 August 1999, the $M_w$ 7.4 İzmit earthquake occurred, as documented by \cite{Altinoketal2001,Inanetal2007}. This seismic event was significant in terms of its magnitude and impact, reflecting the geological vulnerabilities of the region. Regarding the seismic risk in the Marmara segment, Parsons (2004) identified a significantly heightened probability (approximately 50\% depending on the model used) of a magnitude $M \geq 7$ event affecting the region within a 30-year window following the 1999 stress transfer \cite{Parsons2004}. On July 21, 2017, an earthquake with a magnitude of $M_w$=6.6 occurred in the Gulf of Gökova at a depth of approximately 6 km, triggering a tsunami that reached a maximum run-up height of 1.9 m in Bodrum \cite{Tiryakiogluetal2017,Doganetal2019}.
...
Research and technological development in Earthquake Early Warning (EEW) systems have become critical components of seismic risk mitigation, particularly in the seismically active Anatolian region. Current methodologies emphasize the integration of real-time seismic signals with automated alert protocols to protect urban centers and critical infrastructure. For instance, the Kandilli Observatory and Earthquake Research Institute (KOERI) has recently operationalized a new-generation EEWS across T{\"u}rkiye that leverages a dense network of stations to issue alerts within seconds of a primary wave detection, as seen in the $M_w$~5.0 Marmara Sea event \cite{Ozeletal2026}. Furthermore, innovative societal safety strategies now explore hybrid models, such as using existing mosque loudspeakers to disseminate warnings to residential areas where traditional mobile infrastructure may be insufficient \cite{Eren2025}. The integration of impact-based forecasting and machine learning is also transforming tectonic understanding into actionable protocols \cite{Doganetal2019}. These systems go beyond simple detection by predicting potential damage zones and automatically triggering safety actions, such as slowing high-speed trains or shutting down critical utilities \cite{CremenGalasso2020, Specialeetal2026}.

The adoption of big data processing enables the simultaneous analysis of massive streams from dense seismic networks, satellite geodesy, and even crowdsourced smartphone sensors to identify subtle precursors and map crustal deformation in high resolution. By leveraging cloud computing and distributed data architectures, monitoring centres can perform real-time "nowcasting" of ground motion, allowing for more precise risk assessment in EEW and faster dissemination of life-saving alerts across vast urban landscapes in Marmara region.
...
As a result of hybrid approach, geophysicists integrate edge computing for real-time data compression and automated AI quality control for trace picking \cite{Sitetal2021}. Such approach ensures geological accuracy while bypassing the "non-uniqueness" of traditional methods \cite{MousaviBeroza2022}. Hence, the industry has shifted from labor-intensive manual workflows to an asynchronous, "digital twin" approach that can reconstruct complex subsurface structures with unprecedented speed and precision \cite{Zhongetal2023}.
